...
exports.getCleaning = async (req, res)=>{
  ...
  bzDB( { req, res, col:'bzTokens', act:"FIND_ONE", query:{bzToken} }, (userData)=>{
    ...
    if (object?.GET_JOBS) {
      ...
      let query = { "author":login, "date": { $gte:firstDay, $lte:lastDay } }
        ...
        function sortJobs(jobs){
          return jobs.map( day => ({
            ...
            schedule: jobsData.result
              ...
              .sort( (a, b) => {
                const [aHours, aMinutes] = (a?.time?.from || "00:00").split(":").map(Number)
                const [bHours, bMinutes] = (b?.time?.from || "00:00").split(":").map(Number)
