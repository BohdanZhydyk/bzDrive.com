...
export function LeftPannel({ props:{doc, color, checkImg, title, cellStyles, EDIT_DOC} }) {
  ...
  const client = doc?.client?.name ?? (doc?.client?.contacts?.tel ?? ``)
  ...
  return(
    <div className="LeftPannel flex wrap">
      ...
      <div className="DocLeft flex start" title={title()}>
