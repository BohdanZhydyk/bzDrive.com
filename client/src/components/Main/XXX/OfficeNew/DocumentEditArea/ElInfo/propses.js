...
export const LocationPropses = ({tr, lang, location, setLocation, editErr, setEditErr, setSave})=> [
  {
    classes:"zip",
    legend: tr(`ZipLegend`,lang),
    type: `text`,
    val: location?.addr?.zip ? sanitizeTxt(location.addr.zip, `ZIP`).sanText : '',
    err: editErr?.locationAddrZIP ?? '',
    cbVal: (val)=>{
      setLocation( (prev)=> ({...prev, addr:{...prev?.addr, zip:sanitizeTxt(val, `ZIP`).sanText}}))
      setSave(true)
    },
    cbErr: (val)=> setEditErr( (prev)=> ({...prev, locationAddrZIP:sanitizeTxt(val, `ZIP`).sanErr}))
  },
  {
    classes:"town",
    legend: tr(`TownLegend`,lang),
    type: `text`,
    val: location?.addr?.town ? sanitizeTxt(location.addr.town, `town`).sanText : '',
    err: editErr?.locationAddrTown ?? '',
    cbVal: (val)=>{
      setLocation( (prev)=> ({...prev, addr:{...prev?.addr, town:sanitizeTxt(val, `town`).sanText}}))
      setSave(true)
    },
    cbErr: (val)=> setEditErr( (prev)=> ({...prev, locationAddrTown:sanitizeTxt(val, `town`).sanErr}))
  },
  {
    classes:"street",
    legend: tr(`StreetLegend`,lang),
    type: `text`,
    val: location?.addr?.street ? sanitizeTxt(location.addr.street, `StreetName`).sanText : '',
    err: editErr?.locationAddrStreet ?? '',
    cbVal: (val)=>{
      setLocation( (prev)=> ({...prev, addr:{...prev?.addr, street:sanitizeTxt(val, `StreetName`).sanText}}))
      setSave(true)
    },
    cbErr: (val)=> setEditErr( (prev)=> ({...prev, locationAddrStreet:sanitizeTxt(val, `StreetName`).sanErr}))
  },
  {
    classes:"nr",
    legend: tr(`StreetNrLegend`,lang),
    type: `text`,
    val: location?.addr?.nr ? sanitizeTxt(location.addr.nr, `default`).sanText : '',
    err: editErr?.locationAddrNr ?? '',
    cbVal: (val)=>{
      setLocation( (prev)=> ({...prev, addr:{...prev?.addr, nr:sanitizeTxt(val, `default`).sanText}}))
      setSave(true)
    },
    cbErr: (val)=> setEditErr( (prev)=> ({...prev, locationAddrNr:sanitizeTxt(val, `default`).sanErr}))
  }
]
